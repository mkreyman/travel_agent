...
│  │                    Test Job                               │  │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────────┐   │  │
│  │  │   Format    │  │  Compile    │  │     Credo       │   │  │
│  │  │   Check     │──│  (warnings  │──│    (strict)     │   │  │
│  │  │             │  │  as errors) │  │                 │   │  │
│  │  └─────────────┘  └─────────────┘  └─────────────────┘   │  │
│  │         │                                    │            │  │
│  │         └──────────────┬─────────────────────┘            │  │
...
- **Push to main/master**: Full pipeline (test → deploy)
- **Pull requests**: Test job only (no deployment)
...
| `FLY_API_TOKEN` | (GitHub secret) Fly.io deployment token |

### Docker Build

Uses multi-stage Dockerfile generated by `mix phx.gen.release --docker`:

1. **Build stage**: Compiles Elixir release with `MIX_ENV=prod`
2. **Runtime stage**: Minimal Debian image with OTP runtime only
